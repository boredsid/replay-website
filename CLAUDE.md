...
- 2026-06-07 — Phase 3B shipped: admin Editions (list/create/edit) + Users (list/search/edit name-email-notes/view regs+orders/change-phone) screens (`admin/src/pages/Editions*.tsx`, `Users*.tsx`, drawers), worker `/api/admin/editions*` + `/api/admin/users*` handlers, and an edition selector + base-price hint on manual-add. **Migration 003 dropped `editions_only_one_current`**, so "current edition" is now resolved as the latest-`start_date` PUBLISHED edition (tiebreak `created_at desc`) — and this resolver lives in **TWO places that must stay in sync**: `worker/src/editions.ts` (runtime: register/spots/lead/ICS/admin) AND `src/lib/data.ts` (build-time: the static site). If you change one, change the other; otherwise the live site and worker can resolve different editions, and the old `.maybeSingle()` site resolver would crash when >1 row is `is_current`. **Why it matters:** don't reintroduce a single-current uniqueness constraint (admins may flip several editions' `is_current`); `is_current` is now informational, not the resolver key.
...
- 2026-06-08 — Editions are NOT fixed at 2 days. replay-1 is a real 1-day edition (`pricing.oneshot={day1:800}`, `campaign:null`, `capacity_per_day={day1:50}`); 2/3+ days are allowed. The number of days = inclusive span of `start_date..end_date`. `Pricing.oneshot` is a `day1..dayN` map (worker `pricing.ts` + admin `EditionPricing`), and `campaign` is `number|null` (null for single-day). `readPricing`/`readCapacity` accept a variable day map (require at least `day1`); admin `EditionDrawer` derives the day inputs from the date span. **Why it matters:** never reintroduce a hardcoded `oneshot.day2`/`capacity_per_day.day2` requirement — it breaks editing replay-1 (the original Phase-3B bug). **Still 2-day-only (deferred, see HANDOFF "configurable event length — full"):** the PUBLIC register flow (`worker/src/validation.ts` `KNOWN_DAYS=['day1','day2']`, register day selectors), capacity gating, `getConfirmedSeatsByDay`, and the schedule Sat/Sun tabs all still assume exactly 2 days. The current edition (replay-3) is 2-day so the live public flow is unaffected; the stopgap only made admin edit + worker validation day-count-agnostic.
...
- 2026-06-08 — The CF Pages deploy hook `01e9488c-…` (worker secret `CLOUDFLARE_PAGES_DEPLOY_HOOK`, `replay-website` site project) was bound to the deleted phase-0 branch and 400'd (`"Unable to find a branch"`) → admin "Rebuild site" + `POST /api/admin/rebuild` failed with `deploy_hook_failed`. **Resolved same day:** the organiser repointed the SAME hook (id unchanged) to `main` in the CF dashboard — POST now returns `200 {result:{id}}`. No worker secret change/redeploy was needed because the hook id didn't change. **Why it matters:** deploy hooks are bound to a branch; after a branch rename/cutover, repoint the hook (dashboard) rather than assuming the URL is stale. If the hook id ever DOES change, update the worker secret via `cd worker && npx wrangler secret put CLOUDFLARE_PAGES_DEPLOY_HOOK`.
