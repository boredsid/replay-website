...
- 2026-06-08 — The CF Pages deploy hook `01e9488c-…` (worker secret `CLOUDFLARE_PAGES_DEPLOY_HOOK`, the `replay-website` site project) is bound to a now-deleted branch and returns `400 "Unable to find a branch with the provided name"` → the admin "Rebuild site" button + `POST /api/admin/rebuild` fail with `deploy_hook_failed`. Since the Phase-1D cutover the site builds from `main`. **Fix:** create a NEW deploy hook on the `replay-website` Pages project targeting `main` and `wrangler secret put CLOUDFLARE_PAGES_DEPLOY_HOOK` (in `worker/`) with the new URL; the Phase-1B-noted hook URL in HANDOFF is stale. **Why it matters:** the rebuild button can't work until the secret points at a hook bound to an existing branch.
