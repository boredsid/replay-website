...
- 2026-06-02 — Phase 3A admin shipped: worker `/api/admin/*` route group gated in `worker/src/index.ts` by `verifyAccessJwt` reading the `Cf-Access-Jwt-Assertion` header; handlers live in `worker/src/admin/` (whoami, rebuild, dashboard, registrations list/get/create/patch, leads, audit) and respond via `adminJson`/`adminCorsHeaders` (credentialed CORS — specific origin echoed + `Allow-Credentials: true`, NEVER `*`). The SPA is in `admin/` (Vite+React+shadcn, single admin role — no guest concept), port-and-adapt of `bgc-website/admin`. **Why it matters:** the gate 401s every admin call unless Cloudflare injects the JWT header, which only happens when the request path is inside a CF Access app — so the api-admin path MUST stay covered by Access. We added `api.replaycon.in/api/admin` as a SECOND Application Domain on the EXISTING `admin.replaycon.in` Access app (one app → one AUD `0983cd2a…132f` = worker `[vars] CF_ACCESS_AUD`, one shared session cookie across both subdomains). Path-scope it to `/api/admin` so public paths (`/api/health`, `/api/register`, `/api/lead`, `/api/ics/*`) stay OUTSIDE Access. Leave the Access app's own CORS settings OFF — the worker owns CORS; enabling Access CORS too gives duplicate `Access-Control-Allow-Origin` headers and the browser rejects the response.
