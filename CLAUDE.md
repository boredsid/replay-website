...
- 2026-05-31 — replay-2 had 21 walk-in registrations with blank phone/email but real names. Per organiser decision they import under a single placeholder `users.phone = '0000000000'` (name null), with each person's real name preserved in `registrations.source ->> 'guest_name'` for later correction. The mapper rule (`resolvePhone` in `scripts/lib/mappers.ts`): any row whose phone fails `sanitizePhone` → placeholder phone + `guest_name`; real-phone rows are unchanged (no `guest_name` key). **Why it matters:** (1) "unique attendees" can't be counted off distinct phones — 21 confirmed regs share one user row. (2) This deviates from the original spec (which said skip bad-phone rows); the placeholder keeps attendance counts accurate. (3) Re-running the import won't fix the phones — corrections happen in-DB keyed by `guest_name` (see HANDOFF pending "Correct replay-2 walk-in placeholder phones").
