...
import { handleCheckInSearch, handleCheckIn, handleCheckInBulk, handleCheckInUndo, handleAttendeePatch, handleCheckInRoster, handleCheckInTotals, handleCheckInArrivals } from './admin/check-in';
  ...
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      if (path.startsWith('/api/admin/')) {
        ...
        if (path === '/api/admin/dashboard' && req.method === 'GET') return await handleDashboard(req, env, sb, origin, hasFullAccess(staff.roles, '/api/admin/finance'));
        ...
        // Full admins only; the gate is the ADMIN_ONLY prefix in `roles.ts`,
        // checked above with every other route rather than here.
        if (path === '/api/admin/check-in/arrivals' && req.method === 'GET') return await handleCheckInArrivals(req, env, sb, email, origin);
        ...
        if (path === '/api/admin/library/withdrawn' && req.method === 'GET') return await handleLibraryWithdrawn(sb, origin);
